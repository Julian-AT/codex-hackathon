...
async function loadResolver(): Promise<ResolverModule | null> {
	try {
		return (await import(
			/* @vite-ignore */ new URL('./mlx-home.ts', import.meta.url).href
		)) as ResolverModule;
	} catch {
		return null;
	}
...
describe('resolveMlxHome', () => {
	it.each([undefined, '', ' ', '\t\n'])(
		'defaults an absent or blank override (%j)',
		async (value) => {
			const module = await loadResolver();
			expect(module, 'MLX_HOME resolver module exists').not.toBeNull();
			if (!module) return;
			expect(
				module.resolveMlxHome({ env: { MLX_HOME: value }, homedir: () => '/home/operator' }),
			).toEqual({
				ok: true,
	...
	it.each(['relative/root', '~/.mlx', './state'])(
		'rejects a non-absolute override without expanding it: %s',
		async (value) => {
			const module = await loadResolver();
			expect(module, 'MLX_HOME resolver module exists').not.toBeNull();
			if (!module) return;
	...
	it.each(['/tmp/mlx root/../owned', '/tmp/MLX-Δ/./state'])(
		'lexically normalizes absolute spaces and Unicode without filesystem access: %s',
		async (value) => {
			const module = await loadResolver();
			expect(module, 'MLX_HOME resolver module exists').not.toBeNull();
			if (!module) return;
