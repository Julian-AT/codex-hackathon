...
async function queryEndpoint(endpoint: string, prompt: string): Promise<ModelResponse> {
...
async function evaluateTeacher(items: EvalItem[]): Promise<EvalSummary> {
	if (!process.env.MLX_SERVER_URL && !process.env.LOCAL_MODEL) {
		return {
			key: 'teacher',
			label: 'Teacher',
			available: false,
			...
			total: items.length,
			latencyMs: null,
			notes: 'No local model configured',
		};
	}
