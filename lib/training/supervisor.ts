  ...
  ingest(pt: TrainPoint): SupervisorSignal {
    if (pt.reward !== undefined && Number.isFinite(pt.reward)) {
      ...
      if (this.rewards.length === VARIANCE_WINDOW) {
        const mean =
          this.rewards.reduce((sum, reward) => sum + reward, 0) /
          VARIANCE_WINDOW;
        const variance =
          this.rewards.reduce((sum, reward) => sum + (reward - mean) ** 2, 0) /
          VARIANCE_WINDOW;
    ...
    this.emaLoss =
      this.emaLoss === null ? pt.loss : 0.9 * this.emaLoss + 0.1 * pt.loss;
    return { kind: 'continue' };
  }
  ...
  private escalate(reason: 'nan' | 'spike'): SupervisorSignal {
    if (this.rollbacks >= MAX_ROLLBACKS) {
      return {
        kind: 'abort',
        reason:
          reason === 'nan' ? 'nan.unrecoverable' : 'spike.unrecoverable',
      };
    }
    return {
      kind: 'rollback',
