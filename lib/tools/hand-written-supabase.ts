...
export const HAND_WRITTEN_SUPABASE_TOOLS: DynamicToolSpec[] = [
	...
	// ── 2. supabase_select_query_builder ─────────────────────────────
	{
		...
		function: {
			...
			description: 'Build a parameterized SELECT query from table, columns, and filters.',
			parameters: {
				type: 'object',
				properties: {
					table: { type: 'string' },
	...
	// ── 3. supabase_storage_path_builder ─────────────────────────────
	{
		...
		function: {
			...
			description: 'Build a Supabase Storage object path from bucket, userId, and filename.',
			parameters: {
				type: 'object',
				properties: {
					bucket: { type: 'string' },
		...
		meta: {
			...
			trajectories: [
				{
					userPrompt:
						'Build a storage path for user abc-123 uploading photo.png to avatars bucket.',
					call: {
						name: 'supabase_storage_path_builder',
	...
	// ── 4. supabase_edge_function_name_validator ─────────────────────
	{
		...
		function: {
			...
			description: 'Validate a Supabase Edge Function name against Deno-compatible naming rules.',
			parameters: {
				type: 'object',
				properties: {
					name: { type: 'string' },
	...
	// ── 8. supabase_migration_filename ───────────────────────────────
	{
		...
		meta: {
			...
			trajectories: [
				{
					userPrompt:
						'Generate migration filename for "Add users table" at timestamp 1700000000000.',
					call: {
						name: 'supabase_migration_filename',
