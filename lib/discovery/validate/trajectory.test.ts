...
describe('validateTrajectories', () => {
	it('passes when all 3 trajectories match', async () => {
		const r = await validateTrajectories(body, [
			{
				userPrompt: 'q1',
				call: { name: 'addNumbers', arguments: { a: 1, b: 2 } },
				result: { sum: 3 },
			},
			{
				userPrompt: 'q2',
				call: { name: 'addNumbers', arguments: { a: 10, b: 5 } },
				result: { sum: 15 },
			},
			{
				userPrompt: 'q3',
				call: { name: 'addNumbers', arguments: { a: 0, b: 0 } },
				result: { sum: 0 },
			},
		]);
	...
	it('fails on any mismatch', async () => {
		const r = await validateTrajectories(body, [
			...
			{
				userPrompt: 'q3',
				call: { name: 'addNumbers', arguments: { a: 0, b: 1 } },
				result: { sum: 1 },
			},
		]);
