...
import { type ManifestMeta, copyFallback, writeManifest } from './manifest';
import { type SwarmEvent, designToolsSwarm } from './swarm';
...
export async function runDiscoveryPipeline(
	...
	if (survivors.length < killMin) {
		console.warn(`SWR-08 kill-point: only ${survivors.length} survivors`);
		await copyFallback();
