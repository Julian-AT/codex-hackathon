...
- **Smoke route** — `app/api/smoke/route.ts` fires `generateText` in parallel against `claude-opus-4-7`, `gpt-5`, `gemini-3.1-flash-lite` via `Promise.allSettled`. `runtime='nodejs'` + `dynamic='force-dynamic'`. Error surfacing includes alias, status code, and SDK error message.
...
```json
{
  ...
  "google":    { "ok": true,  "alias": "gemini-3.1-flash-lite",  "text": "pong" }
}
```
